<div align="center">
  <a href="https://snakr-im.vercel.app/">
    <img src="./src/assets/svg/logo3.svg" width="280" alt="Snakr logo" />
  </a>
  
  <p><strong>All your games. One place.</strong></p>

  <p>
    <a href="https://snakr-im.vercel.app/">snakr.com</a>
  </p>
</div>


## About

**Snakr** é uma plataforma de **centralização de bibliotecas de jogos**, criada para unificar, organizar e exibir todos os jogos de um usuário em um único lugar, independentemente da plataforma de origem.

A aplicação permite conectar contas de serviços como **Steam, Epic Games e outras**, reunindo jogos, conquistas, status e atividades em uma interface moderna, rápida e focada em experiência do usuário.  
Além disso, o Snakr centraliza **notícias do mundo dos games em tempo real** e oferece um espaço dedicado para **resgate e acompanhamento de jogos, DLCs e conteúdos gratuitos** disponíveis nas principais plataformas.


## Purpose & Vision

- **Propósito:**  
  Eliminar a fragmentação entre plataformas de jogos, oferecendo um **hub único** onde o jogador possa visualizar sua biblioteca completa, acompanhar progresso, consumir notícias relevantes e aproveitar oportunidades de conteúdo gratuito.

- **Visão de Longo Prazo:**  
  - Tornar-se o principal **game hub pessoal** do usuário.  
  - Centralizar bibliotecas, conquistas, tempo de jogo e atividades.  
  - Agregar **notícias atualizadas do ecossistema gamer** em tempo real.  
  - Facilitar o **resgate e o acompanhamento de jogos, DLCs e benefícios gratuitos**.  
  - Oferecer recursos sociais semelhantes a plataformas tradicionais, porém de forma unificada.  
  - Evoluir para um ecossistema extensível com integrações contínuas.


## Technical Architecture

| Camada | Tecnologia / Ferramenta |
|---|---|
| Frontend | React, React Hooks, React Router, CSS customizado |
| Backend | Node.js, API REST |
| Integrações | APIs externas (Steam, NewsApi, etc.) |
| Hospedagem | Vercel |
| Autenticação | OAuth / Tokens de acesso |
| Gerenciamento de estado | Hooks |
| Deploy | CI/CD automatizado |

A arquitetura foi pensada para permitir **novas integrações de plataformas** sem impacto direto na base do sistema.


## Future Roadmap

- ✅ Estrutura base do front-end e back-end  
- ✅ Deploy automatizado na Vercel  
- ✅ Autenticação completa com plataformas externas  
- ✅ Aplicação mobile
- 🎮 Sincronização automática de bibliotecas  
- 🏆 Conquistas globais e comparativos entre amigos  
- 💬 Chat em tempo real e sistema de presença  
- 📊 Estatísticas avançadas de gameplay  
- 🛡️ Segurança, limites de API e controle de tokens  


## Join the Project

O **Snakr** está aberto a colaborações com desenvolvedores interessados em:

- Frontend (React / UX)  
- Backend (Node.js / APIs / Integrações)  
- Arquitetura e performance  
- Experiência social e features multiplayer

### Entre em contato

[![WhatsApp](https://img.shields.io/badge/WhatsApp-Contato-25D366?logo=whatsapp&logoColor=white)](https://wa.me/5511995786774)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Leonardo%20Silva-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/leeosilvp/)


## License

Este projeto, **Snakr**, é um software proprietário e todos os direitos são reservados a Leonardo Silva.

### O que isso significa na prática?

- **Uso Restrito**: O uso, modificação, distribuição, engenharia reversa ou qualquer outra forma de exploração do Snakr é estritamente proibida sem a permissão prévia e por escrito de Leonardo Silva.

Para mais detalhes, leia o arquivo completo de licença: [LICENSE](./LICENSE).


## Credits & Contributors

O desenvolvimento do **Snakr** é fruto do trabalho colaborativo de pessoas que acreditam no poder da tecnologia para conectar, informar e evoluir experiências digitais.

* [Leonardo Silva](https://www.linkedin.com/in/leeosilvp/) - Fundador e **Lead Developer**, responsável pela arquitetura, implementação e visão estratégica do Snakr.